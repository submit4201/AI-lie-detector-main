...
{  event: "analysis.update" | "analysis.done",  service: "transcription" | "audio" | "quantitative" | "manipulation" | "argument" | ...,  payload: { local, gemini, errors, partial, phase, chunkIndex }}
...
{  status: "idle" | "streaming" | "completed" | "error",  transcript: { text, segments, partial },  services: {    [serviceName]: {      current: { local, gemini, errors, partial, phase },      chunks: [ ...history ],    }  },  meta: {    sessionId,    startedAt,    finishedAt,    config,  },  error: null | { message, details }}
Provides actions:
...
If you want, next step I can sketch the exact TypeScript/JS shapes for the v2 SSE payload and the useV2AnalysisSession reducer so implementation is almost mechanical.

Here’s a compact UI‑specific rubric you can use to force the agent (and yourself) to grade and iterate until the frontend is truly production‑ready and aligned with modern design principles (e.g., Material 3).

**V2 Frontend UI Rubric (Target ≥90% Before “Done”)**

For each category, score 0–10. Anything <8 in any category means “iterate again”.

1. **Architecture & API Usage**
...
3. **Modern Design & Visual Language (MD3‑inspired)**
...
7. **Performance & Perceived Latency**
...
8. **Code Quality & Test Coverage (UI)**
...
- Only when overall average ≥9 and no category <8 can the v2 UI be marked “production‑ready” and v1 fully retired.Nice idea. Let’s do a quick brand kit pass focused on identity you can refine later.
...
- **Tone of voice (copywriting):**
  ...
  - Speaks in terms like “indicators”, “signals”, “confidence”,  “you’re lying maybe”.

**2. Name & Tagline Options**

- **Working name:** `VerityScope` 
- **Tagline options:**
...
- **Design principles:**
  - Clarity over drama (no red sirens or “LIAR” stamps unless that flags checked).
...
Use these names in UI labels rather than raw service names. you may  come upwith whatever way to orginizew the information interactive and engaging but the end product must include in some way all metrics 
